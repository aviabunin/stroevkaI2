...
using stroevkaI.Services; // добавлено для доступа к общим сервисам
using stroevkaI.Models;

namespace stroevkaI.Forms
{
    public partial class SredstvaEditor1 : UserControl
    {
        #region Поля
        ...
        public SredstvaEditor1(int subdivisionId) : this()
        {
            this.subdivisionId = subdivisionId;
            LoadSredstvaById(subdivisionId);
        }

        // Конструктор с PivotRow (новый)
        public SredstvaEditor1(PivotRow row) : this()
        {
            if (row != null)
            {
                subdivisionId = row.PchId; // свойство в PivotRow называется Пч? Но мы используем PchId (int)
                LoadSredstvaById(subdivisionId);
            }
        }
